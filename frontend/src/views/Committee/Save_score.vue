<template>
    <v-container>
        <v-row>
            <v-col cols="12">
                <v-form @submit.prevent="saveScore" v-if="user.status_commit === 'n'">
                            ...
                            <v-row v-for="(indicate,index2) in topic.indicates" :key="indicate.id_indicate">
                                <v-col cols="12">
                                    <v-card class="pa-2">
                                        ...
                                        <p class="mt-4">คำอธิบายเพิ่มเติม : {{ indicate.detail_eva || '--' }}</p>
                                        <p v-if="indicate.file_eva" class="mt-4">ไฟล์ที่แนบ : <v-btn @click="openFile(indicate.file_eva)" color="blue" size="small">ดูไฟล์</v-btn></p>
                                        <p v-else class="mt-4 text-red">ไฟล์ที่แนบ : ไม่มีไฟล์</p>
                    ...
                    <div class="mt-4">
                        ...
                        <v-textarea label="คำอธิบายเพิ่มเติม(ถ้ามี)" v-model="detail_commit" rows="2"></v-textarea>
                    </div>
